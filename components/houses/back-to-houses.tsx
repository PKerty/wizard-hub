...
export function BackToHouses({ fromHouseId }: BackToHousesProps) {
  ...
  return (
    <Link
      href="/houses"
      onClick={handleClick}
      className="inline-flex items-center gap-2 font-display text-eyebrow uppercase tracking-[0.2em] text-moonlight transition-colors duration-base ease-arcane hover:text-torchlight"
    >
      ← Back to Houses
    </Link>
  );
